...
export default async function AccountDetailHeaderTabs(
  ...
  const detailLink = "/dashboard/account" as
    | UrlObject
    | __next_route_internal_types__.RouteImpl<URL>
  const settingsLink = "/dashboard/account/settings" as
    | UrlObject
    | __next_route_internal_types__.RouteImpl<URL>
  const agencyLink = "/dashboard/account/agency" as
    | UrlObject
    | __next_route_internal_types__.RouteImpl<URL>
  const helpLink = "/dashboard/account/help" as
    | UrlObject
    | __next_route_internal_types__.RouteImpl<URL>
  const links = {
    Details: detailLink,
  ...
  return (
    <div className="flex rounded bg-slate-300 flex-row gap-1 lg:gap-1.5 p-1 lg:p-1.5 self-center my-2 lg:my-3">
      {(
        ["Details", "Settings", "Agency", "Help"] as AccountDetailHeaderTab[]
      ).map((tab) => (
